...
int ArgumentProcessorResourcesRemove::exec(const std::vector<std::string> &vRoutes, const std::vector<std::string> &vSubParams) {
    ...
    for (int i = 0; i < vResources.size(); i++) {
        ...
        if (sPathModified.rfind(sDirpath1, 0) == 0) {
            vFilesToRemove.push_back(sPath);    
        }
        if (sPath.rfind(sDirpath0, 0) == 0) {
            vFilesToRemove.push_back(sPath);    
        }
    }

    if (vFilesToRemove.size() == 1) {
        if (pkg.hasResource(vFilesToRemove[0])) {
            if (pkg.removeResource(vFilesToRemove[0])) {
        ...
        for (int i = 0; i < vFilesToRemove.size(); i++) {
            std::cout << "Try remove resource '" + vFilesToRemove[i] + "'" << std::endl;
            if (pkg.hasResource(vFilesToRemove[i])) {
                pkg.removeResource(vFilesToRemove[i]);
            } else {
                std::cout << "Resource '" + vFilesToRemove[i] + "' - not found" << std::endl;
            }
        }
        pkg.updateAutogeneratedFiles();
        pkg.save();
