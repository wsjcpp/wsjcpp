...
bool WsjcppPackageManager::reinstall(const std::string &sPackage, std::string &sError) {
    ...
    WsjcppPackageManagerDependence dep;
    if (!m_pDownloaders->downloadToCache(sPackageUrl, m_sRootDir, dep, sError)) {
        return false;
    }
...
bool WsjcppPackageManager::installFromCache(
    ...
    std::string sInstallationDir = WsjcppCore::doNormalizePath(m_sRootDir + "/" + dep.getInstallationDir());

    if (!WsjcppCore::dirExists(sInstallationDir)) {
        WsjcppCore::makeDir(sInstallationDir);
    }

    std::string sCacheDir = m_sRootDir + "/.wsjcpp/cache";
...
void WsjcppPackageManager::recursive_printAuthorsTree(std::vector<WsjcppPackageManagerDependence> &vDependencies) {
  for (int i = 0; i < vDependencies.size(); i++) {
    ...
    if (WsjcppCore::dirExists(dep.getInstallationDir())) {
      WsjcppPackageManager subpkg(sInstalledDir, m_sRootDir, true);
...
bool WsjcppPackageManager::addResource(const std::string &sFilepath, const std::string &sPackAs) {
  ...
  if (!m_yamlPackageInfo.getRoot()->hasElement("resources")) {
    m_yamlPackageInfo.getRoot()->createElementMap("resources");
  }
  ...
  if (!m_yamlPackageInfo.getRoot()->getElement("resources")->hasElement("files")) {
    WsjcppYamlNode *pResources = m_yamlPackageInfo.getRoot()->getElement("resources");
    pResources->setElementValue("output-generation-dir", m_sDirnameResources);
    pResources->createElementArray("files");
  }

  if (!m_yamlPackageInfo.getRoot()->getElement("resources")->hasElement("output-generation-dir")) {
    WsjcppYamlNode *pResources = m_yamlPackageInfo.getRoot()->getElement("resources");
    pResources->setElementValue("output-generation-dir", m_sDirnameResources);
  }

  WsjcppYamlNode *pItem = m_yamlPackageInfo.getRoot()->getElement("resources")->getElement("files");
  ...
  pNewItemMap->setElementValue("size", std::to_string(resourceFile.getFilesize()), WSJCPP_YAML_QUOTES_NONE, WSJCPP_YAML_QUOTES_NONE);
...
bool WsjcppPackageManager::removeResource(const std::string &sFilepath) {
  ...
  } else if (pResources->isMap()) { // > v0.2.6
    if (pResources->hasElement("files")) {
      WsjcppYamlNode *pFiles = pResources->getElement("files");
      int nLen = pFiles->getLength();
      // TODO WsjcppYaml findArrayElementWhereHas("filepath", sValue);
      for (int i = 0; i < nLen; i++) {
        WsjcppYamlNode *pResource = pFiles->getElement(i);
        if (pResource->getElement("filepath")->getValue() == sFilepath) {
          pFiles->removeElement(i);
          bFound = true;
          break;
        }
      }
      if (nLen == 1 && bFound) {
        pResources->removeElement("files");
      }
    }
  }
  if (!bFound) {
    WsjcppLog::err(TAG, "Not found this resource (1)");
    return false;
  }
...
bool WsjcppPackageManager::readFieldResources() {
  ...
  if (cur.isArray()) { // <= v0.2.6
    ...
    return true;
  } else if (cur.isMap()) { // > v0.2.6
    if (cur.hasKey("output-generation-dir")) {
      m_sDirnameResources = cur["output-generation-dir"].valStr();
      m_sDirResources = WsjcppCore::doNormalizePath(m_sRootDir + "/" + m_sDirnameResources);
    }
    if (cur.hasKey("files") && cur["files"].isArray()) {
      int nSize = cur["files"].size();
      for (int i = 0; i < nSize; i++) {
        WsjcppPackageManagerResourceFile resource;
        resource.fromYAML(cur["files"][i].node(), m_bHolded);
        m_vResourceFiles.push_back(resource);
      }
    }
    return true;
...
void WsjcppPackageManager::removeDependenciesFilesSafe(const WsjcppPackageManagerDependence &dep) {
    WsjcppPackageManager pkgDep(dep.getInstallationDir(), m_sRootDir, true);
    if (pkgDep.load()) {
...
bool WsjcppPackageManager::updateAutogeneratedFiles_CMakeListsTXT() {
    ...
    std::string sCMakeListsTXT = ""
        "# Automatically generated by wsjcpp@" + m_sWsjcppCurrentVersion + "\n"
    ...
    for (int iDep = 0; iDep < m_vDependencies.size(); iDep++) {
        ...
        WsjcppPackageManager pkg(dep.getInstallationDir(), m_sRootDir, true);
        if (pkg.load()) {
            sCMakeListsTXT +=
...
bool WsjcppPackageManager::updateAutogeneratedFiles_VersionFile() {
    ...
    std::string sFileName = WsjcppCore::doNormalizePath(m_sRootDir + "/" + m_sVersionFile_Path + "/" + m_sVersionFile_Filename);

    // TODO check sub path must be equal m_sRootDir
...
bool WsjcppPackageManager::updateAutogeneratedFiles_UnitTests() {
    ...
    std::string sBuildSimpleShPath = sUnitTestsDir + "/build_simple.sh";
    WsjcppCore::writeFile(sBuildSimpleShPath, this->getSampleForBuildSimpleSh());

    WsjcppFilePermissions perm(0x775);
    std::string sError;
    if (!WsjcppCore::setFilePermissions(sBuildSimpleShPath, perm, sError)) {
        WsjcppLog::err(TAG, "Could not set permissions for file '" + sBuildSimpleShPath + "'");
    }
...
bool WsjcppPackageManager::updateAutogeneratedFiles_Resource(const WsjcppPackageManagerResourceFile &resFile) {
    ...
    std::string sHeaderContent =
        "// automatically generated by wsjcpp:" + std::string(WSJCPP_APP_VERSION) + "\n"
        "\n"
        "#include <wsjcpp_core.h>\n"
        "\n"
    ...
    std::string sSourceContent =
        "// automatically generated by wsjcpp:" + getVersion() + "\n"
        "\n"
